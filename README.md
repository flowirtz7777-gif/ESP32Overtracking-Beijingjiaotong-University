...
详见 [`预测模型与对照模型构建思路.md`](预测模型与对照模型构建思路.md)（顶层建模哲学）、[`CLAUDE.md`](CLAUDE.md) / [`AGENTS.md`](AGENTS.md)（工程执行手册）。

## 验证策略（Phase 1 核心）

把旧 PID 仿真离散化输出的 `(v, α, φ)` 序列**当作"虚拟传感器测量"**喂给新预测系统，从而在没有真车的条件下完成主模型的几何与时序自洽性验证：

```
旧 PID 仿真 ──┐
   生成 v,α,φ │ → 新主预测系统 → 三层扫掠多边形
   离散序列   │   (短时耦合运动学)   ↓
              │                     与 PID 真值轨迹对比验证
```

这条验证链同时让 PID 模型扮演"工况生成器"和"对照模型一"双重角色。
