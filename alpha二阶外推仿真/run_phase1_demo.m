...
%     run_dir       : 本次结果归档目录绝对路径
    ...
    for ii = 1:numel(keyframes)
        k  = keyframes(ii);
        s0 = truth(k, :);
        % [方案 D · 二阶外推] α(τ)=α₀+α̇·τ+½·α̈·τ²，α̇/α̈ 均 LPF 后估计。
        % 二阶项对噪声敏感，LPF 时间常数取 0.35s（比方案 A 更强）。
        [alpha_dot, alpha_ddot] = estimate_alpha_derivs(alpha, k, dt, 0.35);
        polys.W{ii} = predict_swept(s0, alpha(k), v(k), p, horizons.W, dt_pred, alpha_dot, alpha_ddot);
        polys.A{ii} = predict_swept(s0, alpha(k), v(k), p, horizons.A, dt_pred, alpha_dot, alpha_ddot);
        polys.I{ii} = predict_swept(s0, alpha(k), v(k), p, horizons.I, dt_pred, alpha_dot, alpha_ddot);
    end
