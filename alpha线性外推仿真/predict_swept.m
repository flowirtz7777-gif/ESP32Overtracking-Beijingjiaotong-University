...
%PREDICT_SWEPT  [方案 A · 线性外推] 挂车右侧外缘在 [0, T_h] 扫过区域 → 闭合多边形。
%
%   α 演化假设：α(τ) = α_now + alpha_dot · τ  （线性外推）
%   调用方须传入 **LPF 后** 的 alpha_dot（见 estimate_alpha_dot.m）。
%   alpha_dot = 0 时退化为方案 0（恒定保持）。
%
...
%     T_h       预测时间窗 (s)
%     dt_pred   预测内部步长 (s)
%     alpha_dot LPF 后的转向角速率 (rad/s)
%
%   输出：
%     poly  闭合简单多边形 (N×2)，最后一行 = 第一行
%
%   多边形构造与基线一致（H_right 弧 + 起止直车身边 + T_right 反弧，
%   polyshape 偶奇规则剔除外甩三角）。唯一区别是 α(τ) 随 τ 线性变化。
